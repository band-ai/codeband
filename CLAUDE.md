...
1. **ClaudeSDK/Codex adapters** — Every agent that uses an LLM (Planner, Conductor, Coder, Plan Reviewer, Code Reviewer, Mergemaster) wraps `ClaudeSDKAdapter` or `CodexAdapter` from `band-sdk` (imported from the `band` module namespace, e.g. `from band.adapters import ClaudeSDKAdapter`). band-sdk 1.0.0 renamed the old `thenvoi` SDK namespace to `band`, and band-sdk 2.x replaced the `thenvoi_rest` REST client with `band-client-rest` (module `band_rest`). Import REST types from `band.client.rest`, which re-exports the common ones (`AsyncRestClient`, `ChatMessageRequest`, `ParticipantRequest`, `NotFoundError`); reach into `band_rest.*` only for what that wrapper omits (`ApiError`, `AgentRegisterRequest`, `CreateMyChatRoomRequestChat`). Neither `thenvoi` name may reappear — `tests/test_band_namespace.py` fails the build if it does. They expose an `.adapter` property for `Agent.create()`. Every role supports both Claude and Codex frameworks. Permissions are role-specific: Coders, Mergemaster, and Code Reviewer run with full access (`bypassPermissions` for Claude, `danger-full-access` for Codex) — Coders and Mergemaster in git worktrees, the Code Reviewer in an isolated scratch directory. Coordination-only agents (Plan Reviewer, Claude Conductor) use `permission_mode="dontAsk"` with `approval_mode=None` (Claude still accepts `None` here; Codex does not — see below) — the Claude CLI honors the worktree's `.claude/settings.json` allow list (see profiles in `workspace/init.py`) and deterministically denies everything else. The Codex Conductor gets the closest available equivalent via `sandbox="read-only"` + `approval_policy="never"` + an isolated temporary `cwd` outside the repo. That keeps native Codex file tools away from project files, but it is still weaker than Claude's true "MCP tools only" shape because the Band Codex adapter does not expose a native-tool allowlist.

    Two band-sdk 2.x rules bite here and are easy to regress:
    - `CodexAdapterConfig.approval_mode` is now `Literal["auto_accept", "auto_decline", "manual"]` and rejects `None`; its default `"manual"` would block a headless run waiting on a human. Codeband sets it per privilege level — `auto_accept` for the `danger-full-access` roles, `auto_decline` for the `read-only` ones. `CodexAdapterConfig.cwd` likewise rejects `None` (it defaults to `os.getcwd()`).
    - Adapters no longer take `features=AdapterFeatures(...)`; pass `emit=` and `capabilities=` directly. `Emit.EXECUTION` is now `Emit.TOOL_CALLS`. Critically, `emit` defaults to everything the adapter supports but **`capabilities` defaults to none** — an adapter built without an explicit `capabilities={Capability.MEMORY}` silently loses the memory tools while its prompt still tells the agent to call `band_store_memory`. `tests/test_adapter_features.py` pins this for both frameworks.

    Prompts are loaded via the shared `agents/prompts.py:load_prompt()` utility and are framework-portable — the `band_*` MCP tool names are injected by both adapters. See `player_claude.py` / `player_codex.py` as the canonical coding pair and `conductor.py` as the coordination example.
